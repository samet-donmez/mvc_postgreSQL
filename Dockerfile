...
WORKDIR /src
COPY ["mvc_postgresql.csproj", "."]
RUN dotnet restore "./mvc_postgresql.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./mvc_postgresql.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Bu aşama, son aşamaya kopyalanacak hizmet projesini yayımlamak için kullanılır
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./mvc_postgresql.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "mvc_postgresql.dll"]